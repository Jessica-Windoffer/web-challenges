...
export async function fetchNewColor() {
  ...
  // Task 1: Use the fetch() function to get data from the Color API.
  const colorApiUrlFetch = await fetch(colorApiUrl);

  // Task 2: Use the .json() method to parse the body of the response
  // and convert it into an object.
  const data = await colorApiUrlFetch.json();

  // Task 3: Inspect the parsed object (e.g. by logging it to the console)
